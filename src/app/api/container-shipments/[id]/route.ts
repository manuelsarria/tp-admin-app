...
export async function GET(
  ...
  try {
    ...
    const record = await prisma.containerShipment.findUnique({
      where: { id: params.id },
      include: {
        company: {
          select: {
    ...
    console.error('GET /api/container-shipments/[id] error:', error)
...
export async function PATCH(
  ...
  try {
    ...
    const record = await prisma.containerShipment.update({
      where: { id: params.id },
      data: {
    ...
    console.error('PATCH /api/container-shipments/[id] error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
...
export async function DELETE(
  ...
  try {
    const session = await getServerSession(authOptions)
    if (!session || !['ADMIN', 'WORKER'].includes(session.user.role)) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    await prisma.containerShipment.delete({
      where: { id: params.id },
    })

    return NextResponse.json({ success: true, message: 'Shipment deleted successfully' })
  } catch (error) {
    console.error('DELETE /api/container-shipments/[id] error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
