...
function buildEmailHtml(data: SendPayload): string {
  ...
  const bookingRows = data.bookings.map(b => `
    <tr>
      <td style="padding:10px 14px;border-bottom:1px solid #0A0A0A;font-weight:600;color:#FACC15;font-family:monospace;">${b.hblNumber}</td>
      <td style="padding:10px 14px;border-bottom:1px solid #0A0A0A;">${b.clientName}</td>
      <td style="padding:10px 14px;border-bottom:1px solid #0A0A0A;text-align:center;">${b.packages}</td>
      <td style="padding:10px 14px;border-bottom:1px solid #0A0A0A;text-align:center;">${b.cbm?.toFixed(3) || '—'}</td>
      <td style="padding:10px 14px;border-bottom:1px solid #0A0A0A;">${b.description || '—'}</td>
    </tr>
  `).join('')
...
<body style="margin:0;padding:0;background:#F8FAFC;font-family:Arial,Helvetica,sans-serif;">
  <div style="max-width:680px;margin:0 auto;background:#fff;">
    <!-- Header -->
    <div style="background:linear-gradient(135deg,#FAFAF9,#FACC15);padding:32px 40px;text-align:center;">
    ...
    <div style="padding:32px 40px;">
      ...
      <div style="background:#F8FAFC;border-radius:12px;padding:24px;margin-bottom:28px;border:1px solid #E5E7EB;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
          <h2 style="margin:0;font-size:18px;color:#FAFAF9;">Contenedor ${data.containerNumber || data.mblNumber}</h2>
        ...
        <table style="width:100%;font-size:14px;color:#A8A29E;">
          <tr>
      ...
      <!-- Bookings Table -->
      <h3 style="margin:0 0 12px;font-size:15px;color:#FAFAF9;">Detalle de Carga</h3>
      <table style="width:100%;border-collapse:collapse;font-size:13px;color:#D6D3D1;">
        <thead>
