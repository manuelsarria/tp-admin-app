...
const HEADER_FONT = { fontFamily: 'Poppins, sans-serif' } as const;
...
/* ------------------------------------------------------------------ */
/* Local types (kept local on purpose — types.ts is owned elsewhere)   */
/* ------------------------------------------------------------------ */
...
export default function RubrosTab({ year }: { year: number }) {
  ...
  const load = useCallback(
    async (signal: { cancelled: boolean }) => {
      ...
      try {
        const [byUnit, salarioRes, divRes, spendingRes] = await Promise.all([
        ...
        const rows = normalizeUnitRows(byUnit?.data);
        setUnits(rows);
        setTotales({
...
function ScopeHeader({
  ...
  return (
    <Paper sx={{ ...CARD_SX, p: { xs: 2, sm: 2.5 } }}>
      <Box
        sx={{
          display: 'flex',
      ...
      >
        <Box>
          <Typography variant="h6" sx={{ ...HEADER_FONT, fontWeight: 700, color: INK }}>
            Rubros {year}
          </Typography>
          <Typography variant="body2" sx={{ color: GRAY, mt: 0.25 }}>
            Qué deja cada negocio, cuánto te llevas y de dónde sale el dinero.
          </Typography>
        </Box>
